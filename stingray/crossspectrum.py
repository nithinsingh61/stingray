...
class Crossspectrum(object):
    ...
    def _fourier_cross(self, lc_1, lc_2):
        ...
        lc_2: lightcurve.Lightcurve object
            Another light curve to be Fourier transformed. This is the
            reference band.
    ...
    def coherence(self, lc_1, lc_2):

        cross = self._fourier_cross(lc_1, lc_2)
        crosslc_1 = self._fourier_cross(lc_1, lc_1)
        crosslc_2 = self._fourier_cross(lc_2, lc_2)
        coherence = (abs(cross)**2) / (crosslc_1 * crosslc_2)
        return coherence
...
class AveragedCrossspectrum(Crossspectrum):
    ...
    def __init__(self, lc_1, lc_2, segment_size=1, norm='none'):
        ...
        segment_size: float, default 1 second
            ...
            of each Lightcurve object in lc_1 or lc_2 is not an integermultiple
    ...
    def _make_crossspectrum(self, lc_1, lc_2):
        ...
        if isinstance(lc_1, lightcurve.Lightcurve) and \
            ...
            for lc_1_seg, lc_2_seg in zip(lc_1, lc_2):
                cs_sep, nphots_1_sep, nphots_2_sep = \
                    self._make_segment_csd(lc_1_seg, lc_2_seg,
                                           self.segment_size)
